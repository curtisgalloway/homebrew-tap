...
2. **bottle** (macos-13 Intel + macos-14 Apple Silicon) — source-build the
   re-pinned formula with `brew install --build-bottle`, `brew bottle` it, and
   upload the `.bottle.tar.gz` to a `paniolo-<version>` Release **on this
   repo** (that's the bottles' `root_url`). Each arch stashes its bottle JSON
   as a build artifact.
3. **merge** (ubuntu) — `brew bottle --merge` the per-arch sha256s into the
   formula, `brew style --fix`, commit & push.

So a normal release needs nothing here. After it runs, a Mac on a covered
macOS version installs the precompiled bottle — **no Rust toolchain**. A macOS
version newer than the build hosts uses the newest tag ≤ its own; anything with
no matching bottle (or any Linux install) falls back to the source build, which
`depends_on "rust" => :build` makes automatic.
...
- Bottle OS-version coverage = the build runners: **macos-13** (Intel) and
  **macos-14** (Apple Silicon), chosen as the oldest GA runner per arch for
  maximum downward coverage. GitHub is sunsetting Intel (`macos-13`) runners;
  when they go away, drop that matrix leg (Intel Macs then source-build) or
  move it to a self-hosted runner. Apple Silicon Macs older than the build
  host's macOS are not a concern (all are macOS 11+, the runner targets 12+).
